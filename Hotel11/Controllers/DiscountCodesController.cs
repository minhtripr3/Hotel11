...
namespace Hotel11.Controllers
{
    public class DiscountCodesController : Controller
    {
        ...
        {
            var discountCodes = db.Discount1.ToList();
        ...
        public ActionResult Details(string id)
        {
            ...
            Discount1 discount1 = db.Discount1.Find(id);
            if (discount1 == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Create([Bind(Include = "Code,Amount,DisCountID")] Discount1 discount1)
        {
            if (ModelState.IsValid)
            {
                // Kiểm tra xem mã giảm giá đã tồn tại chưa
                if (db.Discount1.Any(d => d.Code == discount1.Code))
                {
                    ModelState.AddModelError("Code", "Mã giảm giá đã tồn tại.");
                    return View(discount1);
                }

                db.Discount1.Add(discount1);
                db.SaveChanges();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Code,Amount,DisCountID")] Discount1 discount1)
        {
            if (ModelState.IsValid)
            {
                // Kiểm tra xem mã giảm giá đã tồn tại chưa
                var existingDiscount = db.Discount1.FirstOrDefault(d => d.Code == discount1.Code && d.DisCountID != discount1.DisCountID);
